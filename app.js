...
async function loadAll(){
  try{
    const progRes = await window.storage.get('programs', true).catch(()=>null);
    DB.programs = progRes && progRes.value ? JSON.parse(progRes.value) : [DEFAULT_PROGRAM];
    if(!progRes){ await window.storage.set('programs', JSON.stringify(DB.programs), true); }
  }catch(e){ DB.programs = [DEFAULT_PROGRAM]; }

  try{
    const appRes = await window.storage.get('applicants', true).catch(()=>null);
    DB.applicants = appRes && appRes.value ? JSON.parse(appRes.value) : [];
  }catch(e){ DB.applicants = []; }

  try{
    const setRes = await window.storage.get('settings', true).catch(()=>null);
    DB.settings = setRes && setRes.value ? JSON.parse(setRes.value) : { admin_password: 'smarttech2026' };
    if(!setRes){ await window.storage.set('settings', JSON.stringify(DB.settings), true); }
  }catch(e){ DB.settings = { admin_password: 'smarttech2026' }; }

  try{
    const cRes = await window.storage.get('app_counter', true).catch(()=>null);
    DB.counter = cRes && cRes.value ? JSON.parse(cRes.value) : { n: 0 };
  }catch(e){ DB.counter = { n: 0 }; }
}
async function saveApplicants(){ await window.storage.set('applicants', JSON.stringify(DB.applicants), true); }
async function savePrograms(){ await window.storage.set('programs', JSON.stringify(DB.programs), true); }
async function saveSettings(){ await window.storage.set('settings', JSON.stringify(DB.settings), true); }
async function saveCounter(){ await window.storage.set('app_counter', JSON.stringify(DB.counter), true); }
...
function attachHandlers(){
  ...
  if(communityBtn) communityBtn.onclick = ()=>showToast('This would link to the SMART TECH Community — connect it to your real community link.');
